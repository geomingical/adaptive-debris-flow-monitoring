...
BASE_PATH = "/content"
out="/content/drive/MyDrive/Colab Notebooks/ML_Mutiai_debris_flow/no_optuna/"
SEQUENCE_LENGTH = 10
EPOCHS  = 15    # 最終重訓用（小幅增加）
...
def run_for_stations(stations, base_path=BASE_PATH):
    ...
    for station in stations:
        ...
        # === Fixed (Default-like) Hyperparameters ===
        # RF / XGB 用 {} 就會走 sklearn / xgboost 預設
        rf_best_params  = {}  # RandomForestClassifier()
        xgb_best_params = {}  # XGBClassifier()

        # LSTM 給一組穩健、接近預設的常用尺寸；不是搜索，是固定值
        lstm_best_params = {
            "lstm_hidden": 128,
            "mlp_hidden":  64,
            "gate_hidden": 32,
        ...
        print("[Fixed Params] RF={}, XGB={}, LSTM={}".format(rf_best_params, xgb_best_params, lstm_best_params))
        ...
        # ====== NEW: build TimeSeriesDataset for LSTM ======
        ...
        sampler = WeightedRandomSampler(
            ...
            replacement=True,
            generator=gen   # 一律用 CPU generator 避免 device mismatch
        )
        ...
        # ----- Drift detection & (late) fine-tune with real hard negatives
        ...
        if drift_result['data']['is_drift']:
            ...
            if not fine_tune_df.empty and len(fine_tune_df['Label'].unique()) > 1:
                fine_tune_dataset = TimeSeriesDataset(fine_tune_df, feature_cols, SEQUENCE_LENGTH)
                fine_tune_loader = DataLoader(fine_tune_dataset, batch_size=lstm_best_params['batch_size'], shuffle=True, num_workers=0)
                optimizer = optim.Adam(final_lstm_model.parameters(), lr=lstm_best_params['lr'] / 10)
                for epoch in range(10):
                    train_loss = train_one_epoch(final_lstm_model, fine_tune_loader, optimizer, criterion, DEVICE)
                    print(f"Fine-tune Epoch {epoch+1}/10: Loss {train_loss:.4f}")
                torch.save(final_lstm_model.state_dict(), f"fine_tuned_onenet_{station}.pth")
            else:
        ...
        ckpt_path = f"fine_tuned_onenet_{station}.pth" if drift_result['data']['is_drift'] else f"best_onenet_{station}.pth"
        final_lstm_model.load_state_dict(torch.load(ckpt_path, map_location=DEVICE))
